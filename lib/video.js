...
async function splitBySize(inputPath, maxBytesPerPart, estimatedDurationSeconds, _depth = 0) {
  ...
  // 85% margin on top of the caller's own margin - segment cut points
  // aren't byte-exact, so this errs toward parts coming in under the cap.
  const segmentSeconds = Math.max(5, Math.floor((maxBytesPerPart * 0.85) / bytesPerSecond));
